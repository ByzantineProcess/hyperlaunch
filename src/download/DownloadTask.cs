...
public class DownloadTask
{
    ...
    public static List<DownloadTask> FromClientJson(ClientManifest clientManifest)
    {
        ...
        if (clientManifest.Logging?.Client != null)
        {
            ...
            if (!File.Exists(loggingDestination))
            {
    ...
    public async Task<long> ExecuteAsync(SemaphoreSlim networkLimiter = null)
    {
        ...
        if (networkLimiter != null)
        {
            ...
            try
            {
                ...
                data = await CacheEverything.SmartGet(Url, true, true);
            }
    ...
    public static void ExtractNatives(ClientManifest clientManifest, string nativesDir, bool forceExtract = false)
    {
        ...
        foreach (var library in clientManifest.ResolveLibraries(os))
        {
            ...
            var jarPath = Path.Combine(BaseLibraryPath, nativeArtifact.Path.Replace('/', Path.DirectorySeparatorChar));
            ...
            foreach (var entry in archive.Entries)
            {
                ...
                if (ext == ".dll" || ext == ".so" || ext == ".dylib" || ext == ".jnilib")
                {
                    ...
                    if (File.Exists(destPath) && !forceExtract)
                    {
                        continue;
                    }
